...
void noise_atap(const u16* noise,u16 n_len,atap_tag* atap)
{
	...
	if((n_len%((8000/1000)*30))!=0)	
	{
		return;
	}
	frm_num=n_len/((8000/1000)*30);
	...
	abs_sum=0;
	for(i=0;i<n_len;i+=((8000/1000)*30))
	{
		n_max=0;
		for(h=0;h<((8000/1000)*30);h++)
		{
	...
	abs_sum/=(n_len/(20*8000/1000));
	max_sum/=frm_num;
	atap->mid_val=mid;
	atap->n_thl=max_sum*1;
	atap->s_thl=abs_sum*11/10;
	atap->z_thl=(20*8000/1000)*2/160/1;
}
...
void VAD(const u16 *vc, u16 buf_len, valid_tag *valid_voice, atap_tag *atap_arg)
{
	...
	for(i=0;i<(buf_len-(20*8000/1000));i+=((20*8000/1000)-(10*8000/1000)))
	{
		frm_con++;

		frm_sum=0;
		for(h=0;h<(20*8000/1000);h++)
		{
			frm_sum+=(*(vc+i+h)>(atap_arg->mid_val))?(*(vc+i+h)-(atap_arg->mid_val)):((atap_arg->mid_val)-*(vc+i+h));
		}
		
		frm_zero=0;
		for(h=0;h<((20*8000/1000)-1);h++)
		{
			if(*(vc+i+h)>=a_thl)			
			{
				last_sig=2;
			}
			else if(*(vc+i+h)<b_thl)	
			{
				last_sig=1;
			}
		...
		
		{
			...
			{
				front_duration++;
				if(front_duration>=80/(20-10)) 
				{
					cur_stus=2; 
					((valid_tag*)(valid_voice+valid_con))->start=(u16*)vc+i-((80/(20-10)-1)*((20*8000/1000)-(10*8000/1000)));
					front_duration=0; 
				}
			}
			else if(cur_stus==3) 
		...
		else 
		{
			...
			{
				back_duration++;
				if(back_duration>=110/(20-10)) 
				{
					cur_stus=0; 
					((valid_tag*)(valid_voice+valid_con))->end=(u16*)vc+i-(110/(20-10)*((20*8000/1000)-(10*8000/1000)))+(20*8000/1000);
					valid_con++;
					if(valid_con==3)
					{
						return;
